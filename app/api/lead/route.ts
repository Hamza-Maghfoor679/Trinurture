            ...
            <tr>
              <td style="padding:28px 32px 8px;font-family:Arial,Helvetica,sans-serif;font-size:16px;line-height:1.65;color:#3D3632;">
                ...
                <p style="margin:0 0 28px;">
                  Download it below, and keep an eye on WhatsApp too — we&rsquo;ll follow up with care there as well.
                </p>
...
export async function POST(request: Request) {
  ...
  if (!sheetDbUrl || !resendApiKey || !fromEmail || !blueprintUrl) {
    console.error("Missing required environment variables for lead capture.");
    return NextResponse.json(
  ...
  try {
    ...
    const { error: emailError } = await resend.emails.send({
      ...
      html: buildConfirmationEmailHtml({ name, blueprintUrl }),
    });
    ...
    if (emailError) {
      ...
      return NextResponse.json(
        {
          error:
            "We saved your details, but couldn't send the email. Please try again or reach out to us.",
        },
        { status: 502 },
      );
    }
